...
`,Jt={"includes/simulation-state.inc.glsl":Qt,"includes/random.inc.glsl":Zt},te={gradient:(e,t)=>`
		vec2 ${e}(ivec2 texel) {
			ivec2 dx = ivec2(1, 0);
			ivec2 dy = ivec2(0, 1);
			return vec2(
	...
	`},oe=re;function Rt(e){return e=e.replace(/#include "([^"]+)"/g,t=>{const i=t.substring(10,t.length-1),n=Jt[i];if(n===void 0)throw Error(`Could not include file '${i}', make sure you've added it to the 'shaderIncludes' constants in utils/webgl.ts`);return n}),e=e.replace(/#pragma codegen .*/g,t=>{const i=t.substring(16),[n,r]=i.split("=").map(l=>l.trim()),[o,a]=r.substring(0,r.length-1).split("("),s=a.split(",").map(l=>l.trim()),u=oe[o.trim()];if(u===void 0)throw Error(`Could not find generator '${u}', make sure you've added it to the 'shaderCodeGenerators' constants in utils/webgl.ts`);return console.log(`Running codegen rule ${o} on ${n} <- ${s}`),u(n,s)}),e}function ae(e,t,i){const n=e.createShader(e.VERTEX_SHADER);if(n===null)return null;const r=Rt(t);if(e.shaderSource(n,r),e.compileShader(n),!e.getShaderParameter(n,e.COMPILE_STATUS)){const u=e.getShaderInfoLog(n);return console.error(`Could not compile WebGL vertex shader. 
